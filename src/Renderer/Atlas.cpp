...
Atlas::Atlas(GPU_Target *renderer, float *_scale)
{
...
void Atlas::draw_texture(GPU_Image *texture, vec2f scale, vec2f pos,
                         bool flip, int size, bool ignore_size)
{
  ...
  GPU_BlitRotate(texture, NULL, m_renderer_ptr, size, size, 0);
}
void Atlas::draw_entity(Entity *p_entity, Camera *p_camera)
{
  ...
  GPU_BlitRect(p_entity->get_texture(), &src, m_renderer_ptr, &dst);
}
                      ...
                      Camera *camera)
{
  ...
  GPU_BlitRect(message, &src, m_renderer_ptr, &dst);
...
void Atlas::draw_from_sheet(Entity *entity, Camera *camera)
{
  ...
  GPU_Rect src = {curr_sprite.x * curr_sprite.xpu,
  ...
  GPU_Rect dst = {static_cast<int>(pos.x) - static_cast<int>(cam_pos.x),
  ...
  GPU_BlitRect(*curr_sprite.texture, &src, m_renderer_ptr, &dst);

  // SDL_SetTextureColorMod(*entity->get_current_sprite()->texture, 255, 255,
  // 255);
}
void Atlas::draw_texture_from_sheet(GPU_Image *texture, vec2f pos,
                                    ...
                                    bool flip, bool ignore_scale)
{
  ...
  GPU_Rect src = {point.x * point.xpu, point.y * point.ypu, point.xpu,
                  point.ypu};
  GPU_Rect dst;
  if (camera != nullptr)
  {
  ...
  GPU_BlitRect(texture, &src, m_renderer_ptr, &dst);
}

GPU_Target *Atlas::get_renderer() { return m_renderer_ptr; }
